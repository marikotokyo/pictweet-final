...
feature 'tweet', type: :feature do
  ...
  scenario 'post tweet' do
    ...
    fill_in 'user_email', with: user.email #let(:user)で作成したユーザーのemailを入力
    fill_in 'user_password', with: user.password
    find('input[name="commit"]').click #ログインボタンをクリック
    expect(current_path).to eq root_path #ルートに移動したことを確かめている
    expect(page).to have_content('投稿する') #ログイン状態で投稿するボタンが表示されることを確認
